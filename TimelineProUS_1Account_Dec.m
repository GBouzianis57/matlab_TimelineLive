...
[ClassesReturns, ClassesNames] = xlsread('AssetClassesReturnsMonthly','Returns2','D3:K1107'); 
...
AccountId = 1; % Number of different accounts, User decides - IN
AccountAllocation = 1; % Allocate the initial capital to the accounts - IN
...
ScaleInflationRule = [2 3];

ContributionAccount = 1;
ContributionStartAge{1} = 70;
ContributionAmmount{1} = 10000;
ContributionAdjust = 1;
...
OngoingChargePercentage = 0.01/12;
OngoingChargeAmmount = 500/12;

AccountTaxPercentage = 0.2/12 ;
IncomeTaxPercentage = 0.3/12 ;
...
InitialPercentageAllocation = [0 0 0 0.5 0.5 0 0 0]; 
...
%if WithdrawlOrder == 3
    ...
    DrainOrder = [2 1 5 5 5 5 5 5]; % -IN
%end

 %if WithdrawlOrder == 4
    DrainOrderBuckets = [1 2 7 8 3 4 5 6];
    DefaultClass = 6; 
%end
...
%elseif RebalancingRule == 2
    ...
    PerformanceRebalancing = [-1 0 0 0 0 0 0 0]; % -IN
...
%elseif RebalancingRule == 3
 ...
 %elseif RebalancingRule == 4
    ...
    %e2]. 
    ClassesBoundaries = [0.05 0.05 1 1 1 1 1 1]; % -IN
 ...
 %elseif RebalancingRule == 5
    ...
    ClassesPVBoundary = [0.7 0.7 1 1 1 1 1 1];  % -IN
...
GlidingRuleAccount = 0;

GlidingAdjustment = zeros(size(AccountId,2),size(ClassesReturns,2));
GlidingEnd = zeros(size(AccountId,2),size(ClassesReturns,2));
NOGlidingAdjustment = [0 0 0 0 0 0 0 0];
NOGlidingEnd = [0 0 0 0 0 0 0 0];
%If GlidingRuleAccount = 1 choose:
YesGlidingAdjustment = [0 0 0 -0.01/12 0.01/12 0 0 0];
YesGlidingEnd = [0 0 0 0.3 0.7 0 0 0];
